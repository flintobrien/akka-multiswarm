...
// Use the Multi-Objective library (.mo)
//
import com.hungrylearner.pso.particle.breezedvd.mo._
import com.hungrylearner.pso.swarm._
import com.hungrylearner.pso.swarm.Report.ProgressReport
import scala.concurrent.duration._


object BasicMultiObjective {

  class MyReportingStrategy[F,P]( override val parent: ActorRef)
    extends ParentReporter[F,P]
    with PeriodicLocalReporting[F,P]

  class MyMultiLocalSwarmIntelligence[F,P]( override val config: LocalSwarmConfig[F,P], override val childIndex: Int, override val context: ActorContext)
    extends LocalSwarmIntelligence[F,P]
    with MultiEgoImpl[F,P]
    with MultiLocalWorker[F,P]
    with LocalSocialInfluence[F,P]
    with LocalTerminateOnMaxIterations[F,P]
  {
    override protected var _bestPositions = new ParetoFrontDVD
  ...
  class MyMutablePosition( initialPosition: DenseVector[Double], bounds: Array[(Double,Double)]) extends MutablePositionDVD( initialPosition, bounds) {
    ...
    override def evaluateFitness( v: DenseVector[Double], iteration: Int): DenseVector[Double] = {
...
class BasicMultiObjective( iterations: Int) extends Actor with ActorLogging {
  import BasicMultiObjective._
  ...
  override def preStart() = {
    log.debug( "BasicMultiObjective.preStart")
    self ! Initialize
  }
  ...
  def initializePso = {
    log.debug( "BasicMultiObjective.initializePso")
    ...
    if( swarm == null) {
      ...
      val localSwarmIntelligenceFactory = ( childIndex: Int, context: ActorContext) =>
        new MyMultiLocalSwarmIntelligence[Double,DenseVector[Double]]( swarmConfig, childIndex, context)
      ...
      log.error( "BasicMultiObjective.initializePso swarm already initialized!")
    }
  }
...
object Application extends App {
  ...
  private val Logger = Logging.getLogger(system, this)
  Logger.debug( "BasicMultiObjective begin")

  val iterations = 100
  val runPso = system.actorOf( Props( classOf[BasicMultiObjective], iterations),  "runPso")
