...
trait LocalReportingStrategy[F,P] extends ReportingStrategy[F,P]{
  ...
  def reportSwarmingCompleted( childIndex: Int, evaluatedPosition: EvaluatedPosition[F,P], iteration: Int, progress: Progress): Unit
}

trait RegionalReportingStrategy[F,P] extends ReportingStrategy[F,P] {

  def reportForRegion( progressReport: ProgressReport[F,P], childIndex: Int, evaluatedPosition: EvaluatedPosition[F,P], progress: Progress)
}
